...
public class JunkochanControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {//When Space key is pushed while Junkochan is on the ground (Called only once)
        ...
        if (!CheckGrounded() && Input.GetKey(KeyCode.Space) && JumpTime < 0.3f)
        {//When Space key is pushed while Junkochan is in the air (Called while Space key is pushed, no more than 0.3 sec)
